...
namespace task7_1
{
    class Tetrahedron:Primitive
    {
        ...
        public Tetrahedron(List<Point3D> p)
        {
            points = p;
            faces = new List<Face>();
            // Основание тетраэдра
            faces.Add(new Face(new List<Point3D> { points[0], points[1], points[2] }));
            // Левая грань
            faces.Add(new Face(new List<Point3D> { points[1], points[3], points[0] }));
            // Правая грань
            faces.Add(new Face(new List<Point3D> { points[2], points[3], points[1] }));
            // Передняя грань
            faces.Add(new Face(new List<Point3D> { points[0], points[3], points[2] }));
        }
