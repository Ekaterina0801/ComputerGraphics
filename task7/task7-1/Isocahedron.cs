...
namespace task7_1
{
    class Isocahedron:Primitive
    {
        ...
        public Isocahedron(List<Point3D> p)
        {
            points = p;
            faces = new List<Face>();

            for (int i = 0; i < 10; ++i)
                faces.Add(new Face(new List<Point3D> { points[i], points[(i + 1) % 10], points[(i + 2) % 10] }));

            for (int i = 0; i < 5; ++i)
            {
                // верхняя часть
                faces.Add(new Face(new List<Point3D> { points[2 * i], points[10], points[(2 * (i + 1)) % 10] }));
                // нижняя часть
                faces.Add(new Face(new List<Point3D> { points[2 * i + 1], points[11], points[(2 * (i + 1) + 1) % 10] }));
            }
        }
